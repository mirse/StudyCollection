package com.wdz.module_customview.main.materialdesign.testBehavior;
...
public class SampleHeaderBehavior extends CoordinatorLayout.Behavior<TextView> {
    ...
    public void onNestedPreScroll(@NonNull CoordinatorLayout coordinatorLayout, @NonNull TextView child, @NonNull View target, int dx, int dy, @NonNull int[] consumed, int type) {
        ...
        if (target instanceof RecyclerView) {
            ...
            int lastPos = ((LinearLayoutManager) list.getLayoutManager()).findLastVisibleItemPosition();
            int lastCompletelyVisibleItemPosition = ((LinearLayoutManager) list.getLayoutManager()).findLastCompletelyVisibleItemPosition();
            Log.i(TAG, "onNestedPreScroll: pos:"+pos+" lastPosition:"+lastPosition+" lastPos:"+lastPos+" lastCompletelyVisibleItemPosition:"+lastCompletelyVisibleItemPosition);


            if (lastCompletelyVisibleItemPosition==lastPos){
                return;
            }
    ...
    private boolean canScroll(View child, float scrollY) {
        Log.i(TAG, "canScroll: child.getTranslationY():"+child.getTranslationY()+" -child.getHeight():"+-child.getHeight()+" scrollY:"+scrollY+" upReach:"+upReach+" downReach:"+downReach);
