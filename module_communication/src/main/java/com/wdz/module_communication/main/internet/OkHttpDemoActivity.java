package com.wdz.module_communication.main.internet;
...
@Route(path = ARouterConstant.ACTIVITY_OKHTTP)
public class OkHttpDemoActivity extends AppCompatActivity {
    ...
    @OnClick({R2.id.btn_get,R2.id.btn_post,R2.id.bt_startDown,R2.id.bt_pauseDown,R2.id.bt_cancelDown})
    public void onClick(View view){
        int id = view.getId();
        if (id == R.id.btn_get) {//异步get请求
            getData();
        } else if (id == R.id.btn_post) {//post
//                postData();
            testInterceptor();
        } else if (id == R.id.bt_startDown) {
            startDownload();
        ...
        } else if (id == R.id.bt_pauseDown) {
            pauseDownload();
        } else if (id == R.id.bt_cancelDown) {
            cancelDownload();
        }
    }
